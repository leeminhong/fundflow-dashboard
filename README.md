...
### 은행 예금·금전신탁이 월말 며칠간 비어 있어요. (히트맵 빗금 셀)

예금은행·은행신탁 잔액(**실세총예금·실세요구불·저축성·금전신탁**)은 BOK가 **월말~익월 초에 지연 발표**합니다. 일일 게시글에는 해당 셀이 `..`(미입수)로 비어 있다가, 익월 초에 **"잔액"이 포함된 별도 게시글**로 한꺼번에 보정됩니다(대체로 익월 둘째 주경이지만 시점은 매월 다를 수 있음).

- 그 보정 게시글은 일일동향 한 장짜리가 아니라, 빠졌던 날짜마다 **시트가 하나씩** 들어 있는 워크북(예: `5.13`, `4.30` …)입니다.
- 파이프라인은 이를 자동 처리합니다:
  - `parsing.py`가 `일일동향` 시트가 없으면 `M.D` 형태 **날짜 시트를 모두** 읽어 각 레코드에 자기 날짜(`balanceDate`)를 붙입니다.
  - `cli.py`가 BOK RSS 제목에서 `~`와 `잔액`이 함께 든 글을 **보정 게시글로 분류**해, 일일 fetch 창과 별개로 추가 수집합니다(`--backfill-scan`, 기본 60).
  - `webdata.py` 머지에서 **빈 값(None)은 실제 값을 절대 덮어쓰지 않고, 실제 값은 빈 값을 덮어씁니다** → 보정분이 자동 반영됩니다.
- 화면에서는 아직 발표 전 구간을 **빗금(잠정) 셀**로 표시하며, 보정 게시글이 올라오는 대로 다음 갱신에서 실제 값으로 채워집니다. 별도 수동 작업은 필요 없습니다.
